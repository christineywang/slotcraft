...
export function CalendarApp() {
  ...
  useEffect(() => {
    if (!resourceId || panelOpen || adminOpen || dragActive || draggingId) return;
    const id = window.setInterval(() => {
      void refreshBookings(true);
    }, 8000);
    return () => window.clearInterval(id);
  }, [resourceId, panelOpen, adminOpen, dragActive, draggingId, refreshBookings]);
  ...
  async function handleBookingMove(
    booking: Booking,
    startsAt: Date,
    endsAt: Date,
  ) {
    ...
    const prevStartsAt = booking.startsAt;
    const prevEndsAt = booking.endsAt;
    if (
      startsAt.toISOString() === prevStartsAt &&
      endsAt.toISOString() === prevEndsAt
    ) {
      return;
    }
    ...
    try {
      ...
      setBookings((prev) =>
        prev.map((b) =>
          b.id === booking.id
            ? { ...b, startsAt: prevStartsAt, endsAt: prevEndsAt }
            : b,
        ),
      );
    ...
    <main className="mx-auto min-h-screen max-w-7xl px-4 py-5 sm:px-6">
      ...
      <div className="mb-3 flex items-end justify-between gap-3">
        ...
        {loading ? (
          ...
          <span className="text-xs text-ink/45">
            Click a slot · drag a block to reschedule
          </span>
        )}
      </div>
      ...
      {selected ? (
        <WeekCalendar
          ...
          onBookingMove={(booking, startsAt, endsAt) => {
            void handleBookingMove(booking, startsAt, endsAt);
          }}
